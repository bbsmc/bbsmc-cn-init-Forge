...
public class LocalizationNoticeScreen extends Screen {
    ...
    public LocalizationNoticeScreen(JsonObject modpackJson, List<String> languagePacks, File configFile) {
        super(new TextComponent(I18n.get(TITLE_KEY)));
    ...
    protected void init() {
        ...
        this.addRenderableWidget(new Button(startX, buttonY, buttonWidth, buttonHeight,
                new TextComponent(I18n.get("ytongame_hostingmenu.notice.agree")),
                btn -> onAgree()));
        this.addRenderableWidget(new Button(startX + buttonWidth + gap, buttonY, buttonWidth, buttonHeight,
                new TextComponent(I18n.get("ytongame_hostingmenu.notice.decline")),
                btn -> onDecline()));
        ...
        for (String key : NOTICE_KEYS) {
            if (key.isEmpty()) {
                ...
                wrappedLines.addAll(this.font.split(new TextComponent(line), maxWidth));
            }
        }
    }
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        for (FormattedCharSequence line : wrappedLines) {
            if (line != FormattedCharSequence.EMPTY) {
                this.font.drawShadow(poseStack, line, textX, textY, 0xDDDDDD);
            }
            textY += lineHeight;
        }
